...
class Model
{
    ...
    public function checkIfEmpruntExists($empruntID){
        $exists = false;
        $querySelection = $this->pdo->prepare('SELECT COUNT(*) AS Exist FROM emprunts WHERE exemplaire = ? AND fini = 0');
    ...
    public function getHoldNumber($bookId){
            $number = 0;
            $querySelection = $this->pdo->prepare('SELECT COUNT(*) AS Copies FROM exemplaires
   ...
   public function returnEmprunt($empruntId,$dateFin){
    ...
    public function getCopiesNumber($bookId){
        $number = 0;
        $querySelection = $this->pdo->prepare('SELECT COUNT(*) AS Copies FROM exemplaires WHERE book_id = ?');
